...
local friendName = storage.uhrFriend or "PlayerName"
local hpPercent = storage.uhrHp or 60
local runeId = storage.uhrRune or 2273

UI.Separator()

UI.Label("Nome do Player")
UI.TextEdit(friendName, function(widget, text)
  storage.uhrFriend = text
end)

UI.Label("HP% para curar")
UI.TextEdit(hpPercent, function(widget, text)
  storage.uhrHp = tonumber(text)
end)

UI.Label("Rune ID")
UI.TextEdit(runeId, function(widget, text)
  storage.uhrRune = tonumber(text)
end)

macro(200, "Ativar Healing Runes", function()
  local name = storage.uhrFriend or friendName
  local hp = storage.uhrHp or hpPercent
  local rune = storage.uhrRune or runeId

  local target = getCreatureByName(name)

  if target and target:getHealthPercent() <= hp then
    useWith(rune, target)
  end
end)

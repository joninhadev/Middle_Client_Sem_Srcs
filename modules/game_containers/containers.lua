...
function refreshContainerItems(container)
    for slot = 0, container:getCapacity() - 1 do
		...
		local item = container:getItem(slot) -- <<<<<<<<<<<<<< FALTAVA ISSO
        itemWidget:setItem(container:getItem(slot))

        -- Update tier icon
        local tierWidget = itemWidget:getChildById('tierId')
        if tierWidget and item then
            local tierId = item:getTierId()
            if tierId and tierId > 0 and tierId <= 10 then
                tierWidget:setImageSource('/images/ui/tier/object-tier-' .. tierId .. '.png')
...
function toggleContainerPages(containerWindow, hasPages)
    ...
    if hasPages then
        containerWindow.miniwindowScrollBar:setMarginTop(
            containerWindow.miniwindowScrollBar:getMarginTop() + containerWindow.pagePanel:getHeight()
        )
        containerWindow.contentsPanel:setMarginTop(
            containerWindow.contentsPanel:getMarginTop() + containerWindow.pagePanel:getHeight()
        )
    else
        containerWindow.miniwindowScrollBar:setMarginTop(
            containerWindow.miniwindowScrollBar:getMarginTop() - containerWindow.pagePanel:getHeight()
...
function onContainerOpen(container, previousContainer)
    ...
    for slot = 0, container:getCapacity() - 1 do
		...
		-- Create tier widget for container items
		local tierWidget = g_ui.createWidget('UIWidget', itemWidget)
		...
		-- Set initial tier icon if item has tier
    ...
    if not previousContainer then
        ...
        containerWindow:setContentHeight((filledLines * cellSize.height) + 2)
    end
